...
CREATE TABLE `Event` (
    ...
    `emergency_contacts` VARCHAR(191) NOT NULL,
...
CREATE TABLE `LostAndFound` (
    ...
    `claim_User_contact` VARCHAR(191) NOT NULL,
    `reporter_User_name` VARCHAR(191) NOT NULL,
    `reporter_User_contact` VARCHAR(191) NOT NULL,
    `claim_User_name` VARCHAR(191) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `Shop` ADD CONSTRAINT `Shop_event_id_fkey` FOREIGN KEY (`event_id`) REFERENCES `Event`(`event_id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `ShopPayment` ADD CONSTRAINT `ShopPayment_shop_id_fkey` FOREIGN KEY (`shop_id`) REFERENCES `Shop`(`shop_id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Token_Transaction` ADD CONSTRAINT `Token_Transaction_shop_ID_fkey` FOREIGN KEY (`shop_ID`) REFERENCES `Shop`(`shop_id`) ON DELETE RESTRICT ON UPDATE CASCADE;
